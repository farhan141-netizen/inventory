...
with tab_req:
    ...
    with r1: 
        r_item = st.selectbox("Select Product", options=[""] + sorted(meta_df["Product Name"].tolist()) if not meta_df.empty else [""], key="req_sel", label_visibility="collapsed")
    with r2: 
        r_qty = st.number_input("Qty", min_value=0.0, step=1.0, key="req_qty", label_visibility="collapsed")
    with r3:
        if st.button("➕ Create Order", use_container_width=True, type="primary"):
            if r_item and r_qty > 0:
                orders = load_from_sheet("orders_db", ["Date", "Product Name", "Qty", "Supplier", "Status"])
                ...
                save_to_sheet(pd.concat([orders, new_ord], ignore_index=True), "orders_db")
                st.rerun()

    st.markdown('<hr>', unsafe_allow_html=True)
    st.markdown('<span class="section-title">⏳ Pending & Historical Orders</span>', unsafe_allow_html=True)
    
    orders_display = load_from_sheet("orders_db")
    if not orders_display.empty:
        # Ensure Date column exists and sort newest first
        if "Date" not in orders_display.columns: orders_display["Date"] = datetime.datetime.now().strftime("%Y-%m-%d")
        orders_display = orders_display.sort_values(by="Date", ascending=False)
        
        # Interactive Grid for Order Management
        st.data_editor(
            orders_display, 
        ...
        if st.button("💾 Save Order Updates", type="primary"):
            save_to_sheet(orders_display, "orders_db")
            st.rerun()
    else:
        st.info("No orders found. Use the portal above to create one.")
