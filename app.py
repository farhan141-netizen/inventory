        ...
        with col4:
            if st.button("✅ Confirm", use_container_width=True, type="primary", key="receipt_btn"):
                if sel_item and qty_in > 0:
                    ...
                    st.success(f"✅ Added {qty_in} units")
                    st.rerun()
    ...
    with log_col:
        ...
        if not logs.empty:
            for _, row in logs.iloc[::-1].head(10).iterrows():
                ...
                with col_txt:
                    # Fixing formatting string to prevent editor color bleed
                    html_content = f'<div class="log-row {row_class}"><div class="log-info"><b>📦 {row["Item"]}</b><br>Qty: {row["Qty"]} | Day: {row["Day"]}<span class="log-time">[{row["Timestamp"]}]</span></div></div>'
...
with tab_req:
    ...
    with col1: r_item = st.selectbox("🔍 Product", options=[""] + sorted(meta_df["Product Name"].tolist()) if not meta_df.empty else [""], key="req_item")
    with col2: r_qty = st.number_input("📊 Qty", min_value=0.0, key="req_qty")
    with col3:
...
with tab_sup:
    ...
    search = st.text_input("🔍 Search", placeholder="Name or Supplier...")
    if search:
...
# --- SIDEBAR ---
with st.sidebar:
    st.markdown('<h2 class="sidebar-title">☁️ Cloud Control</h2>', unsafe_allow_html=True)
    st.markdown('<p class="section-title">1️⃣ Bulk Inventory Sync</p>', unsafe_allow_html=True)
    inv_file = st.file_uploader("📂 Upload Master", type=["csv", "xlsx"])
    if inv_file:
        try:
            ...
            for i in range(1, 32): new_df[str(i)] = 0.0
            new_df["Total Received"] = 0.0; new_df["Consumption"] = 0.0; new_df["Closing Stock"] = new_df["Opening Stock"]
            if st.button("🚀 Push Inventory", type="primary"):
